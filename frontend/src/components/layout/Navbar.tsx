...
const NAV_STYLES = `
  ...
  .nb-mic-btn {
    ...
    height: 26px;
    border-radius: 50%;
    border: 1px solid rgba(180,120,120,0.35);
    background: transparent;
    cursor: pointer;
    flex-shrink: 0;
    margin-right: 4px;
  ...
  .nb-mic-btn:hover:not(:disabled) { border-color: #c47070; background: #f5e9e9; transform: scale(1.08); }
  .nb-mic-btn:disabled { opacity: 0.45; cursor: not-allowed; }
  .nb-mic-btn.listening {
    border-color: #8b2e2e;
    background: #fff0f0;
    animation: nb-mic-pulse 1s ease-in-out infinite;
  }
  @keyframes nb-mic-pulse {
    0%, 100% { box-shadow: 0 0 0 0 rgba(139,46,46,0.4); }
    50%       { box-shadow: 0 0 0 5px rgba(139,46,46,0); }
  }
  .dark .nb-mic-btn { border-color: rgba(200,74,90,0.35); color: #c84a5a; }
  .dark .nb-mic-btn:hover:not(:disabled) { border-color: #c84a5a; background: rgba(200,74,90,0.1); }
  .dark .nb-mic-btn.listening { border-color: #c84a5a; background: rgba(200,74,90,0.12); }
...
export default function Navbar() {
  ...
  const [isNavListening, setIsNavListening] = useState(false);
  ...
  const startNavListening = () => {
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      setShowSugg(false);
      setSearchLoading(true);
      window.dispatchEvent(new CustomEvent("navbar-ai-search", { detail: { query: text } }));
      const el = document.getElementById("ai-search");
      if (el) el.scrollIntoView({ behavior: "smooth" });
      setSearchLoading(false);
    };
        ...
        >
          ...
          <div className="nb-desktop-nav" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1.8rem" }}>
            ...
            <div ref={searchWrapRef} className="nb-search-wrap-desktop" style={{ position: "relative" }}>
              <div className="nb-search-wrap">
                ...
                {/* ── Mic button (small) ── */}
                <button
                  className={`nb-mic-btn${isNavListening ? " listening" : ""}`}
                  onClick={startNavListening}
                  disabled={searchLoading || isNavListening}
                  aria-label={isNavListening ? "Listening…" : "Voice search"}
                  title={isNavListening ? "Listening…" : "Search by voice"}
                >
                  {isNavListening ? (
                    <svg width="9" height="9" viewBox="0 0 12 12" fill="none">
                      <rect x="2" y="2" width="8" height="8" rx="1.5" fill="#8b2e2e"/>
                    </svg>
                  ) : (
                    <svg width="10" height="10" viewBox="0 0 14 14" fill="none">
                      <rect x="4.5" y="1" width="5" height="7" rx="2.5" stroke="currentColor" strokeWidth="1.3"/>
                      <path d="M2 6.5A5 5 0 0 0 12 6.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/>
          ...
          >
            ...
            <button
              ...
              className="nb-link hidden sm:inline-block"
              style={{ fontSize: "0.65rem", fontWeight: 600, letterSpacing: "0.13em", textTransform: "uppercase", color: "#8A6060" }}
            >
              Support
            </button>
